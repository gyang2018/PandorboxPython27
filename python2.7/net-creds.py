...
logging.basicConfig(filename='../out/credentials.txt',level=logging.INFO)
DN = open(devnull, 'w')
...
def iface_finder():
    try:
        ipr = Popen([ 'route'], stdout=PIPE, stderr=DN)
...
def printer(src_ip_port, dst_ip_port, msg):
    if dst_ip_port != None:
        ...
        for s in skip:
            if s not in msg:
                if os.path.isfile('../out/credentials.txt'):
                    with open('../out/credentials.txt', 'r') as log:
                        contents = log.read()
                        if msg in contents:
                            return
